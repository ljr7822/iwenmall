  ...
  methods:{
    ...
    getCartList(){
      this.axios.get('/carts').then((res)=>{
        ...
        this.cartTotalPrice = res.cartTotalPrice;// 获取商品总金额（选中的）
        this.cartList = list.filter(item=>item.productSelected);// 获取选中的商品
        this.cartList.map((item)=>{
          this.count += item.quantity; // 商品结算件数
        })
      })
    },
    // 打开新增地址弹框
    openAddressModal(){
        this.userAction = 0; // 新增代码
        this.checkedItem = {};
    ...
    // 订单提交
    orderSubmit(){
      let item = this.list[this.checkIndex]; // 通过收货地址列表按照索引寻找地址
      if(!item){
          // 没有取到地址，地址不存在，提示用户
